...
RUN pacman-key --add - < /tmp/qubes-repo-archlinux-key.asc
RUN pacman-key --lsign "$(gpg --with-colons --show-key /tmp/qubes-repo-archlinux-key.asc -| grep ^fpr: | cut -d : -f 10)"
RUN wget -O /tmp/qubes-repo-archlinux-key https://raw.githubusercontent.com/QubesOS/qubes-builderv2/main/qubesbuilder/plugins/chroot_archlinux/keys/qubes-repo-archlinux-key-rdevel.asc
RUN pacman-key --add - < /tmp/qubes-repo-archlinux-key.asc
RUN pacman-key --lsign "$(gpg --with-colons --show-key /tmp/qubes-repo-archlinux-key.asc -| grep ^fpr: | cut -d : -f 10)"
